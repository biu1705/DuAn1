...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($firstname) || empty($lastname)) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            try {
                // Add new user
                $stmt = $conn->prepare("INSERT INTO users (username, email, password, firstname, lastname, address, city, phone, newsletter, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $role = 0; // Set default role as regular user (0)
                
                if ($stmt->execute([$username, $email, $hashed_password, $firstname, $lastname, $address, $city, $phone, $newsletter, $role])) {
